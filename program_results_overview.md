...
AlexNet performs well in distinguishing between dogs and non-dogs with a quick runtime of 23 seconds but struggles with breed accuracy, achieving an 80% success rate in breed classification.
...
VGG excels in both dog identification and breed classification, with a high breed classification accuracy of 93.3%. However, with a runtime of over a minute, it has a longer processing time compared to AlexNet.
...
ResNet provides a good balance between classification accuracy and computational time, with a 90% success rate in breed classification. It correctly identifies dogs and non-dogs but has some misclassifications in breed and non-dog images.
...
AlexNet demonstrates proficiency in identifying dogs and non-dogs within the smaller testing set. **Despite the results not visually confirming it, the model successfully identifies the correct breeds (*refer to the overview*).**
...
Similar to AlexNet, VGG accurately identifies dogs and non-dogs in the smaller testing set but processes the images slower. **Despite the results not visually confirming it, the model successfully identifies the correct breeds (*refer to the overview*).**
...
ResNet performs similarly to AlexNet and VGG on the testing set with a moderate processing speed. **Despite the results not visually confirming it, the model successfully identifies the correct breeds (*refer to the overview*).**
...
3. **Test Images:** All models exhibited a 100% match rate and 100% accuracy in distinguishing between dogs and non-dogs.
...
4. **For Testing with Limited Data:** All architectures had similar performance on the 4-image dataset, demonstrating their robustness in dog and breed classification.
